...
describe "Array#permutation" do
  before :each do
    ...
    @numbers = Grizzly::Collection.new (1..3).to_a
    @yielded = Grizzly::Collection.new([])
  end
  ...
  it "yields all permutations to the block then returns self when called with block but no arguments" do
    ...
    array.should be_an_instance_of(Grizzly::Collection)
  ...
  it "yields all permutations of given length to the block then returns self when called with block and argument" do
    ...
    array.should be_an_instance_of(Grizzly::Collection)
    array.sort.should == @numbers.sort
    @yielded.sort.should == [Grizzly::Collection.new([1, 2]),Grizzly::Collection.new([1, 3]),Grizzly::Collection.new([2, 1]),Grizzly::Collection.new([2, 3]),Grizzly::Collection.new([3, 1]),Grizzly::Collection.new([3, 2])].sort
  end

  it "returns the empty permutation ([[]]) when the given length is 0" do
    @numbers.permutation(0).to_a.should == [Grizzly::Collection.new([])]
    @numbers.permutation(0) { |n| @yielded << n }
    @yielded.should == [Grizzly::Collection.new([])]
  end

  it "returns the empty permutation([]) when called on an empty Array" do
    Grizzly::Collection.new([]).permutation.to_a.should == [Grizzly::Collection.new([])]
    Grizzly::Collection.new([]).permutation { |n| @yielded << n }
    @yielded.should == [Grizzly::Collection.new([])]
  end
  ...
  it "handles duplicate elements correctly" do
    ...
    @numbers.permutation(2).sort.should == [
      Grizzly::Collection.new([1, 1]),Grizzly::Collection.new([1, 1]),Grizzly::Collection.new([1, 2]),Grizzly::Collection.new([1, 2]),Grizzly::Collection.new([1, 3]),Grizzly::Collection.new([1, 3]),
      Grizzly::Collection.new([2, 1]),Grizzly::Collection.new([2, 1]),Grizzly::Collection.new([2, 3]),
      Grizzly::Collection.new([3, 1]),Grizzly::Collection.new([3, 1]),Grizzly::Collection.new([3, 2])
    ].sort
  end
  ...
  it "handles nested Arrays correctly" do
    ...
    # Arrays.
    @numbers << Grizzly::Collection.new([4, 5])
    got = @numbers.permutation(2).to_a
    expected = [
       Grizzly::Collection.new([1, 2]),      Grizzly::Collection.new([1, 3]),      [1, Grizzly::Collection.new([4, 5])],
       Grizzly::Collection.new([2, 1]),      Grizzly::Collection.new([2, 3]),      [2, Grizzly::Collection.new([4, 5])],
       Grizzly::Collection.new([3, 1]),      Grizzly::Collection.new([3, 2]),      [3, Grizzly::Collection.new([4, 5])],
      [Grizzly::Collection.new([4, 5]), 1], [Grizzly::Collection.new([4, 5]), 2], [Grizzly::Collection.new([4, 5]), 3]
    ]
  ...
  it "returns an Enumerator which works as expected even when the array was modified" do
    @numbers = Grizzly::Collection.new([1, 2])
    enum = @numbers.permutation
    @numbers << 3
    enum.to_a.sort.should == [
      Grizzly::Collection.new([1, 2, 3]),Grizzly::Collection.new([1, 3, 2]),Grizzly::Collection.new([2, 1, 3]),Grizzly::Collection.new([2, 3, 1]),Grizzly::Collection.new([3, 1, 2]),Grizzly::Collection.new([3, 2, 1])
  ...
  it "generates from a defensive copy, ignoring mutations" do
    accum = Grizzly::Collection.new([])
    ary = Grizzly::Collection.new([1, 2, 3])
    ary.permutation(3) do |x|
      accum << x
      ary[0] = 5
    end

    accum.should == [Grizzly::Collection.new([1, 2, 3]), Grizzly::Collection.new([1, 3, 2]), Grizzly::Collection.new([2, 1, 3]), Grizzly::Collection.new([2, 3, 1]), Grizzly::Collection.new([3, 1, 2]), Grizzly::Collection.new([3, 2, 1])]
  end
      ...
      describe "size" do
        describe "with an array size greater than 0" do
          ...
          it "returns the descending factorial of array size with array size when there's no param" do
            ...
            Grizzly::Collection.new([1, 2, 3, 4]).permutation.size.should == 24
            Grizzly::Collection.new([1]).permutation.size.should == 1
          end
        end
        describe "with an empty array" do
          it "returns 1 when the given length is 0" do
            Grizzly::Collection.new([]).permutation(0).size.should == 1
          end
          it "returns 1 when there's param" do
            Grizzly::Collection.new([]).permutation.size.should == 1
          end
        end
      end
    end
