...
describe :collection_max_by, shared: true do
  it "returns an enumerator if no block" do
    @subject.new(EnumerableSpecs::Numerous.new(42)).max_by.should be_an_instance_of(Enumerator)
  end

  it "returns nil if #each yields no objects" do
    @subject.new(EnumerableSpecs::Empty.new).max_by {|o| o.nonesuch }.should == nil
  end

  it "returns the object for whom the value returned by block is the largest" do
    @subject.new(EnumerableSpecs::Numerous.new(*%w[1 2 3])).max_by {|obj| obj.to_i }.should == '3'
    @subject.new(EnumerableSpecs::Numerous.new(*%w[three five])).max_by {|obj| obj.length }.should == 'three'
  end

  it "returns the object that appears first in #each in case of a tie" do
    a, b, c = '1', '2', '2'
    @subject.new(EnumerableSpecs::Numerous.new(a, b, c)).max_by {|obj| obj.to_i }.should equal(b)
  end
  ...
  it "uses max.<=>(current) to determine order" do
    ...
    @subject.new(EnumerableSpecs::Numerous.new(a, b, c)).max_by {|obj| obj }.should == a
  end
...
describe "Array#max" do
  ...
  it_behaves_like :collection_max_by, :max_by
  it_behaves_like :enumeratorized_with_origin_size, :max_by, [1,2,3,4]
end
...
describe "Collection#max" do
  ...
  it_behaves_like :collection_max_by, :max_by
  it_behaves_like :enumeratorized_with_origin_size, :max_by, Group.new([1,2,3,4])
end
