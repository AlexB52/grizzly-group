describe :array_slice, shared: true do
  ...
  it "returns the elements specified by Range indexes with [m..n]" do
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 1..3).should == ["b", "c", "d"]
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 4..-1).should == ['e']
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 3..3).should == ['d']
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 3..-2).should == ['d']
    @subject.new(['a']).send(@method, 0..-1).should == ['a']
  ...
  it "returns elements that exist if range start is in the array but range end is not with [m..n]" do
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 4..7).should == ["e"]
  end
  ...
  it "returns nil for a requested index not in the array with [index]" do
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 5).should == nil
  end

  it "returns [] if the index is valid but length is zero with [index, length]" do
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 0, 0).should == []
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 2, 0).should == []
  end

  it "returns nil if length is zero but index is invalid with [index, length]" do
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 100, 0).should == nil
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, -50, 0).should == nil
  end

  # This is by design. It is in the official documentation.
  it "returns [] if index == array.size with [index, length]" do
    @subject.new(%w|a b c d e|).send(@method, 5, 2).should == []
  end

  it "returns nil if index > array.size with [index, length]" do
    @subject.new(%w|a b c d e|).send(@method, 6, 2).should == nil
  end

  it "returns nil if length is negative with [index, length]" do
    @subject.new(%w|a b c d e|).send(@method, 3, -1).should == nil
    @subject.new(%w|a b c d e|).send(@method, 2, -2).should == nil
    @subject.new(%w|a b c d e|).send(@method, 1, -100).should == nil
  end

  it "returns nil if no requested index is in the array with [m..n]" do
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 6..10).should == nil
  end

  it "returns nil if range start is not in the array with [m..n]" do
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, -10..2).should == nil
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, 10..12).should == nil
  end

  it "returns an empty array when m == n with [m...n]" do
    @subject.new([1, 2, 3, 4, 5]).send(@method, 1...1).should == []
  end

  it "returns an empty array with [0...0]" do
    @subject.new([1, 2, 3, 4, 5]).send(@method, 0...0).should == []
  end

  it "returns a subarray where m, n negatives and m < n with [m..n]" do
    @subject.new([ "a", "b", "c", "d", "e" ]).send(@method, -3..-2).should == ["c", "d"]
  end

  it "returns an array containing the first element with [0..0]" do
    @subject.new([1, 2, 3, 4, 5]).send(@method, 0..0).should == [1]
  end

  it "returns the entire array with [0..-1]" do
    @subject.new([1, 2, 3, 4, 5]).send(@method, 0..-1).should == [1, 2, 3, 4, 5]
  end

  it "returns all but the last element with [0...-1]" do
    @subject.new([1, 2, 3, 4, 5]).send(@method, 0...-1).should == [1, 2, 3, 4]
  end

  it "returns [3] for [2..-1] out of [1, 2, 3]" do
    @subject.new([1,2,3]).send(@method, 2..-1).should == [3]
  end

  it "returns an empty array when m > n and m, n are positive with [m..n]" do
    @subject.new([1, 2, 3, 4, 5]).send(@method, 3..2).should == []
  end

  it "returns an empty array when m > n and m, n are negative with [m..n]" do
    @subject.new([1, 2, 3, 4, 5]).send(@method, -2..-3).should == []
  end
  ...
  describe "with a subclass of Array" do
    ...
    it "returns a subclass instance with [n, m]" do
      @array.send(@method, 0, 2).should be_an_instance_of(CollectionSpecs::MyArray)
    end

    it "returns a subclass instance with [-n, m]" do
      @array.send(@method, -3, 2).should be_an_instance_of(CollectionSpecs::MyArray)
    end

    it "returns a subclass instance with [n..m]" do
      @array.send(@method, 1..3).should be_an_instance_of(CollectionSpecs::MyArray)
    end
    ...
    it "returns a subclass instance with [-n..-m]" do
      @array.send(@method, -3..-1).should be_an_instance_of(CollectionSpecs::MyArray)
    end
  ...
  it "raises a type error if a range is passed with a length" do
    ->{ @subject.new([1, 2, 3]).send(@method, 1..2, 1) }.should raise_error(TypeError)
  end
