...
describe :collection_flatten, shared: true do
  it "returns a one-dimensional flattening recursively" do
    @subject.new([[[1, [2, 3]],[2, 3, [4, [4, [5, 5]], [1, 2, 3]]], [4]], []]).flatten.should == [1, 2, 3, 2, 3, 4, 4, 5, 5, 1, 2, 3, 4]
  end

  it "takes an optional argument that determines the level of recursion" do
    @subject.new([ 1, 2, [3, [4, 5] ] ]).flatten(1).should == [1, 2, 3, [4, 5]]
  end
  ...
  it "ignores negative levels" do
    @subject.new([ 1, 2, [ 3, 4, [5, 6] ] ]).flatten(-1).should == [1, 2, 3, 4, 5, 6]
    @subject.new([ 1, 2, [ 3, 4, [5, 6] ] ]).flatten(-10).should == [1, 2, 3, 4, 5, 6]
  end
  ...
  it "tries to convert passed Objects to Integers using #to_int" do
    ...
    @subject.new([ 1, 2, [3, [4, 5] ] ]).flatten(obj).should == [1, 2, 3, [4, 5]]
  end

  it "raises a TypeError when the passed Object can't be converted to an Integer" do
    obj = mock("Not converted")
    -> { @subject.new([ 1, 2, [3, [4, 5] ] ]).flatten(obj) }.should raise_error(TypeError)
  end
  ...
  it "flattens any element which responds to #to_ary, using the return value of said method" do
    ...
    @subject.new([1, 2, x, 5]).flatten.should == [1, 2, 3, 4, 5]

    y = mock("MyArray[]")
    y.should_receive(:to_ary).at_least(:once).and_return(CollectionSpecs::MyArray[])
    @subject.new([y]).flatten.should == []

    z = mock("[2,x,y,5]")
    z.should_receive(:to_ary).and_return([2, x, y, 5])
    @subject.new([1, z, 6]).flatten.should == [1, 2, 3, 4, 5, 6]
  end

  it "does not call #to_ary on elements beyond the given level" do
    obj = mock("1")
    obj.should_not_receive(:to_ary)
    @subject.new([[obj]]).flatten(1)
  end

  it "returns subclass instance for Array subclasses" do
    CollectionSpecs::MyArray[].flatten.should be_an_instance_of(CollectionSpecs::MyArray)
    CollectionSpecs::MyArray[1, 2, 3].flatten.should be_an_instance_of(CollectionSpecs::MyArray)
    CollectionSpecs::MyArray[1, [2], 3].flatten.should be_an_instance_of(CollectionSpecs::MyArray)
    CollectionSpecs::MyArray[1, [2, 3], 4].flatten.should == CollectionSpecs::MyArray[1, 2, 3, 4]
    [CollectionSpecs::MyArray[1, 2, 3]].flatten.should be_an_instance_of(Array)
  end
  ...
  describe "with a non-Array object in the Array" do
    ...
    it "does not call #to_ary if the method is not defined" do
      @subject.new([@obj]).flatten.should == [@obj]
    end
    ...
    it "raises a TypeError if #to_ary does not return an Array" do
      ...
      -> { @subject.new([@obj]).flatten }.should raise_error(TypeError)
    end
...
describe :collection_flatten!, shared: true do
  it "modifies array to produce a one-dimensional flattening recursively" do
    a = @subject.new([[[1, [2, 3]],[2, 3, [4, [4, [5, 5]], [1, 2, 3]]], [4]], []])
    a.flatten!
  ...
  it "does not call flatten! on elements" do
    obj = mock('[1,2]')
    obj.should_not_receive(:flatten!)
    @subject.new([obj, obj]).flatten!.should == nil

    obj = @subject.new([5, 4])
    obj.should_not_receive(:flatten!)
    @subject.new([obj, obj]).flatten!.should == [5, 4, 5, 4]
  end
  ...
  it "raises a FrozenError on frozen arrays when the array would not be modified" do
    -> { CollectionSpecs.frozen_array(@subject).flatten! }.should raise_error(FrozenError)
    -> { CollectionSpecs.empty_frozen_array(@subject).flatten! }.should raise_error(FrozenError)
  end
end
...
describe "Array#flatten" do
  ...
  it_behaves_like :collection_flatten, :flatten
  it_behaves_like :collection_flatten!, :flatten!
...
describe "Collection#flatten" do
  ...
  it "returns subclass instance on Array subclasses" do
    Group.new([1, 2, 3]).flatten.should be_an_instance_of(Group)
    Group.new([1, 2, [3]]).flatten!.should be_an_instance_of(Group)
  end
end