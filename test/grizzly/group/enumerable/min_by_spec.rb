...
describe :collection_min_by, shared: true do
  it "returns an enumerator if no block" do
    @subject.new(EnumerableSpecs::Numerous.new(42)).min_by.should be_an_instance_of(Enumerator)
  end

  it "returns nil if #each yields no objects" do
    @subject.new(EnumerableSpecs::Empty.new).min_by {|o| o.nonesuch }.should == nil
  end

  it "returns the object for whom the value returned by block is the smallest" do
    @subject.new(EnumerableSpecs::Numerous.new(*%w[3 2 1])).min_by {|obj| obj.to_i }.should == '1'
    @subject.new(EnumerableSpecs::Numerous.new(*%w[five three])).min_by {|obj| obj.length }.should == 'five'
  end

  it "returns the object that appears first in #each in case of a tie" do
    a, b, c = '2', '1', '1'
    @subject.new(EnumerableSpecs::Numerous.new(a, b, c)).min_by {|obj| obj.to_i }.should equal(b)
  end
  ...
  it "uses min.<=>(current) to determine order" do
    ...
    @subject.new(EnumerableSpecs::Numerous.new(a, b, c)).min_by {|obj| obj }.should == c
  end
  ...
  context "when called with an argument n" do
    before :each do
      @enum = @subject.new(EnumerableSpecs::Numerous.new(101, 55, 1, 20, 33, 500, 60))
    end
...
describe "Array#min_by" do
  before { @subject = Array }

  it_behaves_like :collection_min_by, :min_by
  it_behaves_like :enumeratorized_with_origin_size, :min_by, [1,2,3,4]
end

describe "Collection#min_by" do
  before { @subject = Group }

  it_behaves_like :collection_min_by, :min_by
  it_behaves_like :enumeratorized_with_origin_size, :min_by, Group.new([1,2,3,4])
end